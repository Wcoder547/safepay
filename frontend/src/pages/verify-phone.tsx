...
export function VerifyPhonePage() {
  ...
  // ── Send OTP — defined BEFORE useEffect that calls it ──
  ...
  const handleChange = (index: number, value: string) => {
    ...
    if (digit && index === OTP_LENGTH - 1) {
      ...
      if (fullCode.length === OTP_LENGTH) handleVerify(fullCode);
    }
  };
            ...
            <>
              ...
              {isSendError && (
                <div className="mb-5 flex items-start gap-2.5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3">
                  ...
                  <p className="text-sm font-medium text-amber-700">{getError(sendError)}</p>
                </div>
              )}
              ...
              {isVerifyError && (
                <div className="mb-5 flex items-start gap-2.5 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3">
                  ...
                  <p className="text-sm font-medium text-rose-700">{getError(verifyError)}</p>
                </div>
              )}
              ...
              <Button
                ...
                className="w-full h-13 rounded-2xl bg-linear-to-r from-blue-600 via-indigo-600 to-purple-600 text-white font-semibold shadow-xl hover:shadow-2xl hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
              >
                {isVerifying ? (
              ...
              <p className="mt-5 text-center text-xs text-slate-400">
                Wrong number?{" "}
                <Link to="/sign-up" className="font-semibold text-blue-600 hover:text-blue-700 transition-colors">
                  Go back and edit
                </Link>
              </p>
            </>
